/*
 * Author : Phuc Le
 *
 * Created on Tue Jan 07 2025
 *
 * Copyright (c) 2025 Acadamic Purpose. All rights reserved.
 */

#define TOTAL_TRUCKS 5
#define MAX_RESEND_MESS 5
#define MAX_BRAKE_FORCE 1.0
#define TRUCK_SAFE_DISTANCE 10.0
#define TRUCK_DUMMY_DISTANCE 10.0
#define TRUCK_BRAKE_SPEED 10.0
#define TRUCK_SPEED_UP_SPEED 5.0
#define OBSTACLE_ALERT_CHANCE 5
#define DEFAULT_SESSION_KEY "PHUC01QUYEN02ISSAC03DAO04"

#define SERVICE_ENV_FILE_DEFAULT "/src/dataset.env"
